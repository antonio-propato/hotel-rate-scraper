...
async function scrapeBookingCom(page, checkIn, checkOut) {
  ...
  try {
    ...
    const rates = await page.evaluate(() => {
      ...
      // DEBUG: First, let's see exactly what's on the page
      console.log('🐛 DEBUG: Extracting all available data...');
      
      // Extract all room names
      const allRoomElements = document.querySelectorAll('.hprt-roomtype-icon-link');
      const allRoomNames = Array.from(allRoomElements).map((el, index) => {
        const name = el.textContent.trim();
        console.log(`Room ${index + 1}: "${name}"`);
        return name;
      }).filter(name => name.length > 2);
      
      // Extract all prices from different selectors
      const priceSelectors = ['.prco-valign-middle-helper', '.bui-price-display__value', 'span'];
      let allPrices = [];
...
(async () => {
  try {
    ...
    process.exit(1);
            ...
            console.log(`Price element (${selector}): "${text}"`);
            
            // Try to extract price value
            let priceMatch = text.match(/£\s*(\d{1,4}(?:,\d{3})*)/);
            if (priceMatch) {
              allPrices.push({
                value: parseInt(priceMatch[1].replace(/,/g, '')),
                currency: 'GBP',
                original: text
              });
              return;
            ...
            priceMatch = text.match(/US\$\s*(\d{1,4}(?:,\d{3})*)/);
            if (priceMatch) {
              const converted = Math.round(parseInt(priceMatch[1].replace(/,/g, '')) * 0.79);
              allPrices.push({
                value: converted,
                currency: 'GBP',
                original: text,
      ...
      const uniquePrices = allPrices.filter((price, index, arr) => 
        arr.findIndex(p => p.value === price.value) === index
      );
      
      console.log(`DEBUG: Found ${allRoomNames.length} room names`);
      console.log(`DEBUG: Found ${uniquePrices.length} unique prices`);
      
      allRoomNames.forEach((name, i) => console.log(`  Room ${i+1}: ${name}`));
      uniquePrices.forEach((price, i) => console.log(`  Price ${i+1}: £${price.value} (${price.original})`));
      
      // Match rooms with prices
      const matchCount = Math.min(allRoomNames.length, uniquePrices.length);
      console.log(`DEBUG: Attempting to match ${matchCount} room/price pairs`);
      
      for (let i = 0; i < matchCount; i++) {
        if (allRoomNames[i] && uniquePrices[i] && uniquePrices[i].value >= 100) {
          console.log(`✅ MATCHED: ${allRoomNames[i]} - £${uniquePrices[i].value}`);
          results.push({
            ota: 'Booking.com',
            roomName: allRoomNames[i],
            price: uniquePrices[i].value,
            currency: 'GBP',
            source: uniquePrices[i].converted ? 'debug-extraction-converted' : 'debug-extraction'
          });
        }
      }
      
      // If we still don't have results, let's try a different approach
      if (results.length === 0) {
        console.log('🔄 DEBUG: Trying table-based extraction...');
        ...
        tableRows.forEach((row, index) => {
          const roomEl = row.querySelector('.hprt-roomtype-icon-link');
          const priceEls = row.querySelectorAll('span, .bui-price-display__value, .prco-valign-middle-helper');
          
          if (roomEl) {
            const roomName = roomEl.textContent.trim();
            ...
            priceEls.forEach(priceEl => {
              ...
              let priceMatch = priceText.match(/£\s*(\d{1,4}(?:,\d{3})*)/);
              if (priceMatch && !foundPrice) {
                foundPrice = parseInt(priceMatch[1].replace(/,/g, ''));
                return;
              }
              
              priceMatch = priceText.match(/US\$\s*(\d{1,4}(?:,\d{3})*)/);
              if (priceMatch && !foundPrice) {
                foundPrice = Math.round(parseInt(priceMatch[1].replace(/,/g, '')) * 0.79);
              }
            });
            
            if (roomName.length > 2 && foundPrice && foundPrice >= 100 && foundPrice <= 5000) {
              console.log(`✅ TABLE ROW: ${roomName} - £${foundPrice}`);
              results.push({
                ota: 'Booking.com',
                roomName: roomName,
                price: foundPrice,
                currency: 'GBP',
                source: 'table-row-debug'
              });
            }
          }
        });
      ...
      console.log(`DEBUG: Final result count: ${results.length}`);
